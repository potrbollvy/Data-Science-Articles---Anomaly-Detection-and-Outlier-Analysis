...
Les observations anormales ne sont pas toujours des signes avant-coureurs de grandes découvertes scientifiques, cependant -- les observations inattendues peuvent gâcher les analyses ou encore indiquer l'existence de problèmes liés à la collecte ou au traitement des données. \par Quoi qu'il en soit, il devient impératif pour les décideurs et les analystes d'établir des protocoles de détection des anomalies et d'identifier des stratégies pour faire face à de telles observations.
\subsection{Notions fondamentales et survol}
Les \textbf{valeurs aberrantes} sont des observations qui sont \textbf{atypical} par rapport aux autres caract\'eristiques de l'unit\'e d'ob\-ser\-vation, ou par rapport aux valeurs correspondantes des autres unit\'es d'observations. Les valeurs aberrantes sont donc des observations qui  \textbf{diff\`erent des autres cas} ou qui contredisent les  \textbf{dependances} (or r\`egles) \textbf{connues} au sujet des donn\'ees.\footnote{Une observation peut \'etre aberrante selon n'importe laquelle des variables, ou encore en combinaison.}% (information in this section is taken partly from \cite{DP_OW,DP_A,DP_T,DP_CBK}).
\par Il se peut qu'une observation soit aberrante ou anormale dans un contexte particulier, mais pas dans un autre. Prenons, par exemple, le cas d'un homme adulte de 180cm de taille. Un tel homme se retrouve dans le 86i\`eme centile par rapport \`a la population masculine canadienne \cite{DP_HPC}, ce qui n'est pas inhabituel (quoique d\'efinitivement au dessus de la moyenne); en Bolivie, cependant, le même homme se retrouve dans le 99,9i\`eme centile \cite{DP_HPC}, ce qui le marque comme un g\'eant en relation avec la population masculine bolivienne. \footnote{La détection d'anomalies peut mener les analystes et les expert.e.s en la mati\`ere \`a se poser d'int\'eressantes questions; dans le contexte de l'exemple de la taille, pourquoi y a-t-il un écart si important entre les deux populations?}
\newl
Les anlaystes commettent parfois une erreur fondamentale lors du traitent des valeurs aberrantes en les retirant de l'ensemble de données sans d\'eterminer au pr\'ealable si ce sont des \textbf{observations influentes}, c'est-à-dire des observations dont l'absence entraîne des résultats d'analyse \textbf{nettement différents}.\par Lorsque de telles observations sont identifiées, on doit d'abord appliquer des mesures correctives (comme les stratégies de transformation des données, entre autres) afin de minimiser tout effet ind\'esirable. Il est \`a noter que les valeurs aberrantes peuvent \^etre influent, et que des observations influentes peuvent être des valeurs aberrantes, mais les conditions ne sont ni nécessaires, ni suffisantes.
\subsubsection*{La d\'etection d'anomalies}  Par d\'efinition, les anomalies sont \textbf{peu fréquentes}, et typiquement enrob\'ees d'\textbf{incertitude} en raison de leur petit nombre, relativement parlant, ce qui complique leur  différenciation d'un simple \textbf{bruit} ou d'\textbf{erreurs de saisie de donn\'ees}. \par De plus, la fronti\`ere entre les unités normales et les unités déviantes est en g\'en\'eral \textbf{floue}; avec l'avènement des boutiques en ligne, par exemple, un achat effectué à 3h du matin (heure locale) ne sonne plus nécessairement l'alarme lors de l'analyse des transactions. \par Il est déjà assez difficile d'essayer d'identifier les anomalies "honnêtes"; lorsque elles sont en fait associées à des \textbf{activités malveillantes}, elles sont typiquement \textbf{déguisées} afin de ressembler à des observations normales, ce qui peut venir compliquer les choses.  
\newl
Il y a plusieurs méthodes pour essayer d'identifier les observations anormales -- \textbf{aucune d'entre elles n'est infaillible}. Les méthodes qui utilisent des aides graphiques (comme les diagrammes de boîtes \`a moustache, les nuages de points, et les tours 2D) pour identifier les valeurs aberrantes sont particulièrement faciles à mettre en oeuvre, mais l'interpr\'etation des graphiques demeurent compliqu\'ees en pr\'esence de donn\'es de haute dimension. \par Il existe aussi des méthodes analytiques (celles qui utilisant les distances de Cooke ou de Mahalanobis, par exemple), mais elles demandent en g\'en\'eral la conduite d'analyses supplémentaires, surtout lors de l'identification de donn\'ees influentes (\textit{cf.} ``\textbf{leverage}''). 
\newl Dans les petits ensembles de données, la détection d'anomalies peut être effectuée au cas par cas, mais avec des ensembles de donn\'ees de grande taille, les analystes se doivent de r'esister \`a la  tentation d'utiliser la \textbf{détection/ suppression automatisée} des anomalies.\footnote{Une fois que les observations "anormales" sont supprim\'es de l'ensemble de données, les observations qui \'etaient ant\'erieurement "régulières" peuvent à leur tour devenir anormales dans le plus petit ensemble de données; quand le processus est automatis\'ee, il n'est pas \'evident qu'il s'arr\^etera avant qu'il ne reste qu'une seule obvservation dans l'ensemble de donn\'ees.}\par
Dans les premières stades de la détection des anomalies, les analyses de donn\'ees  \textbf{simples} (telles que le calcul de statistiques descriptives et de tableaux de contingence, ou encore une repr\'sentation visuelle traditionnelle) peuvent être effectuées afin d'aider  à identifier les observations anormales ou \`a  obtenir un aperçu des données, ce qui pourrait éventuellement entra\^{\i}ner des modifications au plan d'analyse.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.5\textwidth]{Images/decomposition.png}
...
\label{fig:cv_cont}
\end{figure*}
\par The diagnostic plots are shown in Figure~\ref{fig:diff}: the 2010 CV series is prior-adjusted from the beginning until OCT2010 after the detection of a level shift. The SI (Seasonal Irregular) chart shows that there are more than one irregular component which exhibits volatility.
\begin{figure*}[t]
\centering
\includegraphics[width=0.98\textwidth]{Images/DiffComponents.png}
\caption[Diagnostic plots.]{Diagnostic plots. Note here that the analysis of a time series starts with estimation of the effects of festivals and trading days. These pre-calculated
...
\qquad\includegraphics[width=0.4\textwidth]{Images/Confusion.png}\par\noindent
