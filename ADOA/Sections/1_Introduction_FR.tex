...
\subsubsection*{Tests de valeurs aberrantes} Les méthodes de d\'etection se situent dans l'un de deux camps: \textbf{supervis\'ee} et \textbf{non supervis\'ee}.
...
\newpage \noindent Another supervised approach is to estimate the relative abnormality of various ob\-ser\-va\-tions: it is usually quite difficult to estimate the probability that an ob\-ser\-va\-tion $\mathbf{x}_1$ is anomalous with any certainty, but it might be possible to determine that it is more likely to be anomalous than another ob\-ser\-va\-tion $\mathbf{x}_2$, say (denoted by $\mathbf{x}_1\succeq \mathbf{x}_2$). %\par Either way, it is usually preferable to attempt to estimate $P(\mathbf{x}\text{ is an anomaly}|I)$ rather than to attempt to predict $\text{class}(\mathbf{x})\in\{\text{anomaly},\text{normal}\}$ directly. 
