...
\end{center}
\noindent Observations could be anomalous in one context, but not in another. For instance: 
\begin{itemize}
\item an adult male who is 6-foot tall falls in the $86$th percentile among Canadian males $\Rightarrow$ tall, but not unusually so; 
\item in Bolivia, the same man would land in the $99.9$th percentile $\Rightarrow$ extremely tall; a rarity.
\end{itemize}
...
\newpage\ \\ \noindent A common mistake that analysts make when dealing with outlying observations is to remove them from the dataset without carefully studying whether they are \textbf{influential data points}, that is, observations whose absence leads to \textbf{markedly different} analysis results.

When influential observations are identified, remedial measures (such as data transformation strategies) may need to be applied to minimize any undue effect. 

Note that outliers may be influential, and influential data points may be outliers, but the conditions are neither necessary nor sufficient. 


\fh{Anomalies} 

By definition, anomalies are \textbf{infrequent} and typically surrounded by \textbf{uncertainty} due to their relatively low numbers, which makes it difficult to differentiate them from banal \textbf{noise} or \textbf{data collection errors}. 

Furthermore, the boundary between normal and deviant observations is usually \textbf{fuzzy}; with the advent of e-shops, for instance, a purchase which is recorded at 3AM local time does not necessarily raise a red flag anymore. 

When anomalies are actually associated to \textbf{malicious activities}, they are more than often \textbf{disguised} in order to blend in with normal observations, which obviously complicates the detection process.


Numerous methods exist to identify anomalous observations; \textbf{none of them are foolproof} and judgement must be used. 
Methods that employ \textbf{graphical aids} (such as box-plots, scatterplots, scatterplot matrices, and 2D tours) to identify outliers are particularly easy to implement, but a low-dimensional setting is usually required for ease of interpretability. 
...
\begin{figure}[H]
\centering
\includegraphics[width=0.5\textwidth]{Images/mmsc_EN.png}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{Images/clustering2_EN.png}
