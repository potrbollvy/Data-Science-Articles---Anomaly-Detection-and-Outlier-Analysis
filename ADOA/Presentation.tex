...
\textcolor{white}{ab} \localtextbulletone\ Anomalies in Text Datasets (p.\@\pageref{6.5.2})\normalsize 
 \ \\ \ \\ 
...
\fh{6.5.2 -- Anomalies in Text Datasets} \label{6.5.2} 
...
\end{itemize}
\textbf{Word vector representations} help with the first two of these, but at the cost of removing the third (and  reduced interpretability). \newl The proximity-based models that have been discussed previously can be extended to text data, but with subtle differences.    
\fh{Proximity-Based Models}
...
\newpage\ \\ \noindent However the documents have been \textbf{normalized}, the corpus is now represented by a matrix $$\textbf{X}=\begin{bmatrix} \textbf{D}_1 \\ \vdots \\ \textbf{D}_N \end{bmatrix}.$$ The similarity between documents is computed using the \textbf{cosine similarity}: 
$$w(\textbf{D}_i,\textbf{D}_j)=\cos(\textbf{D}_i,\textbf{D}_j)=\frac{\textbf{D}_i\cdot \textbf{D}_j}{\|\textbf{D}_i \| \|\textbf{D}_j \|}.$$
Proximity-based algorithms can then be applied to this dataset, as they would to any numerical dataset, but the results are not always ... satisfactory.  
\end{document}
