...
\noindent With the advent of automatic data collection, it is now possible to store and process large troves of data. There are technical issues associated to massive data sets, such as the speed and efficiency of analytical methods, but there are also problems related to the detection of \textbf{anomalous observations} and the \textbf{analysis of outliers}.  \newl Unexpected observations can spoil analysises and/or be indicative of data collection and data processing issues.\ \\ \ \\  Extreme and irregular values behave very differently from the majority of observations: they can represent criminal attacks, fraud attempts, targeted attacks, or data collection errors. As a result, anomaly detection and outlier analysis plays a crucial role in cyber-security, quality control, etc.   
\newpage\ \\ \noindent 
5.1 -- Basic Notions and Overview (p.\pageref{5.1}) \\ \small  
\textcolor{white}{ab} \localtextbulletone\ Anomaly Detection as Statistical Learning  (p.\@\pageref{5.1.1}) 
\normalsize \ \\ \ \\  
 5.2 -- Quantitative Methods of Anomaly Detection  (p.\pageref{5.2}) \\ \small
\textcolor{white}{ab} \localtextbulletone\ Distance-Based Methods (p.\@\pageref{5.2.1}) \\ 
\textcolor{white}{ab} \localtextbulletone\ Density-Based Methods (p.\@\pageref{5.2.2}) 
\normalsize \ \\ \ \\  
5.3 -- Qualitative Methods (p.\pageref{5.3}) \\ \small
\textcolor{white}{ab} \localtextbulletone\ AVF Algorithm (p.\@\pageref{5.3.1}) \\ 
...
\normalsize
\newpage\ \\ \noindent 
%5.5 -- Applications  (p.\pageref{5.5}) \\ 
%\small
%\textcolor{white}{ab} \localtextbulletone\ S\&P 500 (p.\@\pageref{5.5.1}) \\ 
%\textcolor{white}{ab} \localtextbulletone\ Airports (p.\@\pageref{5.5.2}) \\ 
%\textcolor{white}{ab} \localtextbulletone\ Application 3 (p.\@\pageref{5.5.3}) \\ 
%\textcolor{white}{ab} \localtextbulletone\ Application 4 (p.\@\pageref{5.5.4}) \\ 
%\textcolor{white}{ab} \localtextbulletone\ Application 5 (p.\@\pageref{5.5.5}) 
%\normalsize \ \\ \ \\ 
...
\fh{5.1.1 -- Anomaly Detection as Statistical Learning} \label{5.1.1} 
...
\fh{\textcolor{darkestgreen}{5.2 -- Quantitative Methods of Anomaly Detection}} \label{5.2} 
...
\fh{5.2.1 -- Distance-Based Methods} \label{5.2.1}
...
\fh{5.2.2 -- Density-Based Methods} \label{5.2.2} 
...
\fh{5.3.2 -- Greedy Algorithm} \label{5.3.2} 
...
\fh{5.4.2 -- Projection-Based Methods} \label{5.4.2} 
...
%\fh{\textcolor{darkestgreen}{5.5 -- Applications}} \label{5.5}



%\fh{5.5.1 -- S\&P 500} \label{5.5.1}



%\fh{5.5.2 -- Airports} \label{5.5.2} 



%\fh{5.5.3 -- Application 3} \label{5.5.3} 



%\fh{5.5.4 -- Application 4} \label{5.5.4} 



%\fh{5.5.5 -- Application 5} \label{5.5.5} 
...
\newl Even without combining the results, it may be useful to run multiple algorithms on different subspaces to produce an \textbf{Overview of Outliers}~(O3). 
\newpage \begin{center}
...
\fh{5.5.2 -- Anomalies in Text Datasets} \label{5.5.2} 
\noindent In the \textbf{one-hot encoding} framework, text data is typically
\begin{itemize}
\item \textbf{sparse};
\item \textbf{high-dimensional}, and 
\item \textbf{non-negative}.
\end{itemize}
\textbf{Word vector representations} help with the first two of these, but at the cost of removing the third (and  reduced interpretability). \newl The anomaly detection models that have been discussed previously can be extended to text data, but with subtle differences.    
\fh{Text Processing}
...
\newl 
$\Warning$ There is no sound mathematical justification to use the term frequency-inverse document frequency statistic. It may may not always be the ideal choice, but it is often used nonetheless. \newl  \textbf{Word vector representations} change the nature of DTM.
\fh{Anomaly Detection Models} \noindent However the documents have been \textbf{normalized}, the corpus is now represented by a matrix $$\textbf{X}=\begin{bmatrix} \textbf{D}_1 \\ \vdots \\ \textbf{D}_N \end{bmatrix}.$$ For proximity-based methods, the similarity between documents is computed using the \textbf{cosine similarity}: 
$$w(\textbf{D}_i,\textbf{D}_j)=\cos(\textbf{D}_i,\textbf{D}_j)=\frac{\textbf{D}_i\cdot \textbf{D}_j}{\|\textbf{D}_i \| \|\textbf{D}_j \|}.$$
